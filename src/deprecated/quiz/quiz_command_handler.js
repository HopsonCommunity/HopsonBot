...
module.exports = class QuizEventHandler extends CommandHandlerBase
{
    ...
    {
        if (this.quizActive) {
            Bot.sendMessage(message.channel, `Sorry, a quiz is currently active in ${this.session.channel.name}`);
            return;
        }
    ...
    tryEndQuiz(message, args) 
    {
        ...
        if (!channelHasQuizSession(message.channel)) {   //Cannot end a quiz from a different channel
    ...
    submitAnswer(message, answer)
    {
        ...
        if (!channelHasQuizSession(message.channel)) {
            return;
        }
...
function addQuestion(category, qu, ans, authorID) 
{
    ...
    fs.readFile(questionsFile, 'utf8', (err, data) => {
        ...
        fs.writeFile(questionsFile, qOut, console.log);
    });
}
...
function countQuestions(message)
{
    ...
    for (let question of questions) {
        let val = result.get(question.cat);
